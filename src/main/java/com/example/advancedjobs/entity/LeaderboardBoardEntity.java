...
public class LeaderboardBoardEntity extends Villager implements RoleBasedNpc {
    public LeaderboardBoardEntity(EntityType<? extends Villager> entityType, Level level) {
        super(entityType, level);
        this.setVillagerData(new VillagerData(VillagerType.SAVANNA, VillagerProfession.NITWIT, 5));
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (player instanceof ServerPlayer serverPlayer) {
            ...
            sendLeaderboardHint(serverPlayer, profile, useSecondary, targetJobId);
            if (serverPlayer.isShiftKeyDown()) {
                AdvancedJobsMod.get().jobManager().openScreen(serverPlayer, "my_job", targetJobId);
            } else {
                AdvancedJobsMod.get().jobManager().openScreen(serverPlayer, "top", targetJobId);
            }
        }
    ...
    private void sendLeaderboardHint(ServerPlayer player, PlayerJobProfile profile, boolean secondary, String targetJobId) {
        final int radius = 16;
        ...
        if (targetJobId == null) {
            var entries = AdvancedJobsMod.get().jobManager().overallLeaderboard();
            String leader = entries.isEmpty() ? "-" : entries.iterator().next().playerName();
            player.sendSystemMessage(Component.translatable("message.advancedjobs.leaderboard_board.overall_hint",
                entries.size(),
                leader));
            player.sendSystemMessage(Component.translatable("message.advancedjobs.leaderboard_board.next_step",
                Component.translatable(recommendedRole(profile, secondary).translationKey()),
                guideReason(profile, recommendedRole(profile, secondary), secondary)));
            int nextRoleCount = nearbyBoardCount(player, recommendedRole(profile, secondary), radius);
            if (nextRoleCount > 0) {
                String nextRoleSource = nearestSource(player, recommendedRole(profile, secondary), radius);
        ...
        var entries = AdvancedJobsMod.get().jobManager().leaderboard(targetJobId);
        String leader = entries.isEmpty() ? "-" : entries.iterator().next().playerName();
