...
public class ContractsBoardEntity extends Villager implements RoleBasedNpc {
    public ContractsBoardEntity(EntityType<? extends Villager> entityType, Level level) {
        super(entityType, level);
        this.setVillagerData(new VillagerData(VillagerType.TAIGA, VillagerProfession.NITWIT, 5));
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (player instanceof ServerPlayer serverPlayer) {
            ...
            sendContractsHint(serverPlayer, profile, useSecondary, targetJobId);
            if (serverPlayer.isShiftKeyDown()) {
                var preview = AdvancedJobsMod.get().jobManager().previewContractReroll(serverPlayer, targetJobId);
                if (preview == com.example.advancedjobs.job.JobManager.ContractRerollResult.SUCCESS
                    && AdvancedJobsMod.get().jobManager().rerollContracts(serverPlayer, targetJobId)) {
                    ...
                    var definition = targetJobId == null
                        ...
                        : AdvancedJobsMod.get().jobManager().job(targetJobId);
                    if (definition.isPresent()) {
                        jobName = Component.translatable(definition.get().translationKey());
                    }
                    serverPlayer.sendSystemMessage(Component.translatable("message.advancedjobs.contracts_board_rerolled", jobName));
                } else {
                    serverPlayer.sendSystemMessage(contractRerollFailureMessage(serverPlayer, profile, preview));
                }
            } else {
                AdvancedJobsMod.get().jobManager().openScreen(serverPlayer, "contracts", targetJobId);
            }
        }
        return InteractionResult.CONSUME;
    }

    private Component contractRerollFailureMessage(ServerPlayer player, PlayerJobProfile profile,
                                                  com.example.advancedjobs.job.JobManager.ContractRerollResult result) {
        return switch (result) {
            ...
            case COOLDOWN -> Component.translatable("message.advancedjobs.contracts_board_reroll_cooldown",
                com.example.advancedjobs.util.TimeUtil.formatRemainingSeconds(
                    AdvancedJobsMod.get().jobManager().contractRerollCooldownRemaining(profile)));
            case INSUFFICIENT_FUNDS -> Component.translatable("message.advancedjobs.contracts_board_reroll_insufficient_funds",
                com.example.advancedjobs.util.TextUtil.fmt2(com.example.advancedjobs.config.ConfigManager.COMMON.contractRerollPrice.get()));
    ...
    private void sendContractsHint(ServerPlayer player, PlayerJobProfile profile, boolean secondary, String targetJobId) {
        final int radius = 16;
