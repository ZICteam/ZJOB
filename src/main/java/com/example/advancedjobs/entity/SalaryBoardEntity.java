...
public class SalaryBoardEntity extends Villager implements RoleBasedNpc {
    public SalaryBoardEntity(EntityType<? extends Villager> entityType, Level level) {
        super(entityType, level);
        this.setVillagerData(new VillagerData(VillagerType.DESERT, VillagerProfession.NITWIT, 5));
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (player instanceof ServerPlayer serverPlayer) {
            ...
            sendSalaryHint(serverPlayer, profile, useSecondary, targetJobId);
            if (serverPlayer.isShiftKeyDown()) {
                if (ConfigManager.COMMON.instantSalary.get()) {
                    serverPlayer.sendSystemMessage(TextUtil.tr("command.advancedjobs.salary.auto_mode"));
                } else {
                    double paid = AdvancedJobsMod.get().jobManager().claimSalary(serverPlayer);
                    if (paid < 0.0D) {
                        serverPlayer.sendSystemMessage(TextUtil.tr("command.advancedjobs.salary.cooldown",
                            TimeUtil.formatRemainingSeconds((long) -paid)));
                    } else if (paid > 0.0D) {
                        serverPlayer.sendSystemMessage(TextUtil.tr("command.advancedjobs.salary.claimed", TextUtil.fmt2(paid)));
                    } else {
                        serverPlayer.sendSystemMessage(Component.translatable("message.advancedjobs.salary_board_empty"));
                    }
                }
            } else {
                AdvancedJobsMod.get().jobManager().openScreen(serverPlayer, "salary", targetJobId);
            }
        }
    ...
    private void sendSalaryHint(ServerPlayer player, PlayerJobProfile profile, boolean secondary, String targetJobId) {
        final int radius = 16;
        ...
        long cooldown = AdvancedJobsMod.get().jobManager().salaryClaimCooldownRemaining(profile);
        player.sendSystemMessage(Component.translatable("message.advancedjobs.salary_board.cooldown_hint",
            TimeUtil.formatRemainingSeconds(cooldown)));
