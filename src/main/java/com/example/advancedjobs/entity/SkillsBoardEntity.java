...
public class SkillsBoardEntity extends Villager implements RoleBasedNpc {
    public SkillsBoardEntity(EntityType<? extends Villager> entityType, Level level) {
        super(entityType, level);
        this.setVillagerData(new VillagerData(VillagerType.JUNGLE, VillagerProfession.NITWIT, 5));
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (player instanceof ServerPlayer serverPlayer) {
            ...
            sendSkillsHint(serverPlayer, profile, useSecondary, targetJobId);
            if (serverPlayer.isShiftKeyDown()) {
                AdvancedJobsMod.get().jobManager().openScreen(serverPlayer, "my_job", targetJobId);
            } else {
                AdvancedJobsMod.get().jobManager().openScreen(serverPlayer, "skills", targetJobId);
            }
        }
    ...
    private void sendSkillsHint(ServerPlayer player, PlayerJobProfile profile, boolean secondary, String targetJobId) {
        final int radius = 16;
        ...
        NpcRole followUpRole = followUpRoleAfterSkills(profile, secondary);
        player.sendSystemMessage(Component.translatable("message.advancedjobs.skills_board.next_step",
            ...
            guideReason(profile, followUpRole, secondary)));
            int nextRoleCount = nearbyBoardCount(player, followUpRole, radius);
            if (nextRoleCount > 0) {
                String nextRoleSource = nearestSource(player, followUpRole, radius);
